...
const size = (state = initSizeState, action) => {
    switch (action.type) {
        ...
        case SET_SCENE_HEIGHT: {
            return {
                ...state, sceneInfo: state.sceneInfo.map(
                    (scene) =>  (scene.sceneNum === action.payload.sceneNum && scene.type === action.payload.type) ? ({ ...scene, scrollHeight: action.payload.scrollHeight }) : scene )
            }
        }
